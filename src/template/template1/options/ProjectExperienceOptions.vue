  ...
  <el-tabs v-model="activeName" type="card" class="demo-tabs" stretch>
    ...
    <el-tab-pane label="数据配置" name="data">
      <el-form label-width="70px" label-position="left">
        ...
        <div
          class="work-list"
          v-for="(item, index) in modelItem.LIST"
          :key="index"
        >
          <p>
          ...
          <el-form-item label="公司名称:">
            <el-input
              ...
              show-word-limit
            />
            <el-switch v-model="modelItem.isShow.projectName" />
          </el-form-item>
          ...
          <el-form-item
            label="工作内容:"
            v-for="(content, cIndex) in item.projectContent"
            :key="cIndex"
          >
            <el-input
              type="textarea"
...
<script setup lang="ts">
  ...
  const modelItem = reactive<IPROJECTEXPERIENCE>(
    resumeJsonStore.LIST[index] as IPROJECTEXPERIENCE,
  );
  ...
  const addContent = (index: number): void => {
    modelItem.LIST[index].projectContent.push({
      content: "主要工作内容是打杂",
    });
  };
  // 删除内容
  const deleteContent = (index: number, cIndex: number): void => {
    modelItem.LIST[index].projectContent.splice(cIndex, 1);
  };
</script>
<script lang="ts">
  export default {
